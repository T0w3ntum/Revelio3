...
type RevelioScanner struct{}

func worker(command string, args []string) error {
...
func (s RevelioScanner) Process(r *librevelio.Revelio, ip string, wg *sync.WaitGroup) ([]librevelio.Result, error) {
	...
	for _, h := range data.Hosts {
		...
		for _, address := range h.Addresses {
			ret = append(ret, librevelio.Result{
				FileName: outputfile,
